...
// Already in your imports — just add BarChart2:
import {
  LayoutDashboard, Building2, FileText, Users, UserCircle,
  LogOut, ArrowUpRight, ChevronRight, Menu, X,
  TrendingUp, Eye, PenLine, Plus, RefreshCw, CheckCircle2,
  AlertTriangle, Zap, BarChart2, Globe, Search, Activity,   // BarChart2 already here ✅
  Bell, Loader2, CheckCircle,
} from "lucide-react";
import PropertyManager   from "./sections/PropertyManager_firestore";
...
import { sendNotification, NotifPayload } from "@/lib/sendNotification";
...
export default function AdminDashboard({ onLogout }: AdminDashboardProps) {
  ...
  return (
    <div className="flex min-h-screen bg-slate-50 font-sans">
      ...
      {/* Sidebar */}
      <aside className={`${sideOpen ? "w-56" : "w-14"} bg-white border-r border-slate-200 flex flex-col shrink-0 fixed top-0 left-0 h-full z-50 transition-all duration-300 shadow-sm`}>
        <div className="flex items-center gap-3 px-3 py-4 border-b border-slate-100 min-h-[60px]">
          ...
          {sideOpen && (
            <div className="min-w-0 flex-1">
              <p className="text-slate-800 font-normal text-sm leading-tight">RealHubb</p>
              ...
              <button key={item.id} onClick={() => setActive(item.id)}
                ...
                {sideOpen && (
                  <div className="flex-1 min-w-0">
                    ...
                    {!on && <p className="text-[10px] text-slate-400 leading-tight mt-0.5">{item.desc}</p>}
                  </div>
                )}
        ...
        <div className="px-2 pb-4 pt-3 border-t border-slate-100 space-y-0.5">
          <a href="/" target="_blank" rel="noreferrer"
            ...
            {sideOpen && <><span className="flex-1">View Site</span><ArrowUpRight className="h-3.5 w-3.5 opacity-0 group-hover:opacity-60 transition" /></>}
          </a>
      ...
      {/* Main content */}
      <div className={`flex-1 flex flex-col ${sideOpen ? "ml-56" : "ml-14"} transition-all duration-300 min-h-screen`}>
        <header className="bg-white border-b border-slate-200 px-6 py-3.5 sticky top-0 z-40 shadow-sm">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h1 className="text-sm font-normal text-slate-800 leading-tight">{nav.label}</h1>
    ...
    <div className="bg-white rounded-2xl border border-slate-200 p-6 shadow-sm">
      <div className="flex items-center justify-between mb-5">
        <div className="flex items-center gap-2">
          ...
          <p className="text-sm font-normal text-slate-700">Send Push Notification</p>
        </div>
        {subCount !== null && (
      ...
      <div className="space-y-3">
        <div>
          <label className="text-xs font-normal text-slate-500 uppercase tracking-wide block mb-1">Title *</label>
          <input value={title} onChange={e => setTitle(e.target.value)}
            placeholder="🏠 New Property Alert!"
            className="w-full px-3 py-2.5 text-sm border border-slate-200 rounded-lg bg-slate-50 focus:outline-none focus:ring-2 focus:ring-indigo-300 focus:border-indigo-400 transition" />
        </div>
        <div>
          <label className="text-xs font-normal text-slate-500 uppercase tracking-wide block mb-1">Message *</label>
          <textarea value={body} onChange={e => setBody(e.target.value)}
            placeholder="Ramky Fortuna — 1,2,3 BHK in Whitefield from ₹79L"
            rows={2}
            className="w-full px-3 py-2.5 text-sm border border-slate-200 rounded-lg bg-slate-50 focus:outline-none focus:ring-2 focus:ring-indigo-300 focus:border-indigo-400 transition resize-none" />
        </div>
        <div>
          <label className="text-xs font-normal text-slate-500 uppercase tracking-wide block mb-1">Link (URL)</label>
        ...
        <button
          ...
          className="w-full py-2.5 rounded-xl bg-indigo-500 hover:bg-indigo-600 disabled:opacity-50 text-white text-sm font-normal transition flex items-center justify-center gap-2"
        >
          {sending
...
function OverviewSection({ onNavigate }: OverviewProps) {
  ...
  posts.forEach((p: any) => { const c = p.category || "Other"; catMap[c] = (catMap[c] || 0) + 1; });
        ...
        {statCards.map(s => (
          <button key={s.label} onClick={() => onNavigate(s.nav)}
            ...
            <div className="flex items-center justify-between mb-4">
              <span className={`text-xs font-normal uppercase tracking-wider ${s.color}`}>{s.label}</span>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
        ...
        <div className="bg-white rounded-2xl border border-slate-200 p-6 shadow-sm flex flex-col items-center">
          <div className="flex items-center gap-2 mb-4 self-start">
            ...
            <p className="text-sm font-normal text-slate-700">SEO Content Score</p>
          </div>
        ...
        <div className="bg-white rounded-2xl border border-slate-200 p-6 shadow-sm">
          <div className="flex items-center gap-2 mb-5">
            ...
            <p className="text-sm font-normal text-slate-700">Properties by City</p>
          </div>
          {loading ? (
            <div className="space-y-3">
        ...
        <div className="bg-white rounded-2xl border border-slate-200 p-6 shadow-sm">
          <div className="flex items-center gap-2 mb-5">
            ...
            <p className="text-sm font-normal text-slate-700">Top Blog Categories</p>
          </div>
          {loading ? (
            <div className="space-y-3">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
        ...
        <div className="bg-white rounded-2xl border border-slate-200 p-6 shadow-sm">
          <div className="flex items-center gap-2 mb-5">
            ...
            <p className="text-sm font-normal text-slate-700">Content Completeness</p>
          </div>
          <div className="space-y-4">
            {[
              ...
              { label: "Developers with logos",        value: devs.filter((d: any) => d.logo).length,                        total: devs.length,  color: "bg-amber-500"   },
              { label: "Team members with photos",     value: team.filter((m: any) => m.photo).length,                       total: team.length,  color: "bg-rose-500"    },
            ].map(item => {
              ...
              return (
                <div key={item.label}>
                  <div className="flex justify-between text-xs mb-1.5">
                    ...
                    <span className="text-slate-500 font-medium">{item.value}/{item.total} ({pct}%)</span>
                  </div>
                  <div className="h-2 bg-slate-100 rounded-full overflow-hidden">
        ...
        <div className="bg-white rounded-2xl border border-slate-200 p-6 shadow-sm">
          <div className="flex items-center gap-2 mb-5">
            ...
            <p className="text-sm font-normal text-slate-700">Quick Actions</p>
          </div>
          <div className="grid grid-cols-2 gap-3">
            {[
              ...
              <button key={a.label} onClick={() => onNavigate(a.nav)}
                ...
                <p className={`text-sm font-normal ${a.color}`}>{a.label}</p>
          ...
          <div className="mt-4 p-4 rounded-xl bg-gradient-to-r from-indigo-50 to-violet-50 border border-indigo-100">
            <div className="flex items-start gap-3">
              ...
              <div>
                <p className="text-xs font-normal text-indigo-700 mb-1">SEO tip</p>
...
function ScoreRing({ score }: { score: number }) {
  ...
  return (
    <div className="flex flex-col items-center">
      ...
      <span className="text-sm font-normal mt-1" style={{ color }}>{label}</span>
    </div>
  );
}