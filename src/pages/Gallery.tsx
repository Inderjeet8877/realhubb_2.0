...
import { Images, Search, Calendar } from "lucide-react";
...
const Gallery = () => {
  ...
  const [lightbox, setLightbox] = useState<null | { image: string; title: string; description: string }>(null);
  ...
  return (
    <>
      <SEO
        ...
        description="Explore RealHubb's gallery — property showcases, team moments, awards, and events from our real estate journey across Bangalore, Hyderabad and Chennai."
      ...
      {lightbox && (
        ...
        >
          ...
          >
            <img src={lightbox.image} alt={lightbox.title} className="w-full max-h-[65vh] object-contain bg-slate-900" />
            <div className="p-5">
              <h3 className="text-[#00274D] font-normal text-lg mb-1">{lightbox.title}</h3>
              {lightbox.description && (
                <p className="text-gray-400 text-sm leading-relaxed">{lightbox.description}</p>
              )}
            </div>
          </div>
        </div>
      ...
      <div className="min-h-screen bg-[#faf6f1]">
        ...
        <section className="pt-32 pb-20 bg-[#00274D]">
          <div className="px-8 md:px-14 lg:px-20 xl:px-28">
            <FadeInOnScroll direction="up">
              ...
              <h1 className="text-4xl md:text-[56px] font-normal text-white leading-tight mb-4 max-w-2xl">
        ...
        {/* ── FILTERS ── */}
        <section className="py-8 bg-[#faf6f1] sticky top-16 z-10 border-b border-gray-100 shadow-sm">
          <div className="px-8 md:px-14 lg:px-20 xl:px-28">
            <div className="flex flex-col sm:flex-row gap-4 items-start sm:items-center">
              ...
              <div className="relative w-full sm:w-72">
                ...
                <input
                  ...
                  className="w-full pl-9 pr-4 py-2 text-sm border border-gray-200 rounded-full bg-white focus:outline-none focus:ring-2 focus:ring-[#D7A764]/30 focus:border-[#D7A764]"
                />
              </div>
                ...
                {filtered.map((post, index) => (
                  <FadeInOnScroll key={post.id} direction="up" delay={Math.min(index * 60, 300)}>
                    <div
                      ...
                      onClick={() => setLightbox({ image: post.image, title: post.title, description: post.description })}
                    >
                      {/* Image */}
                      <div className="relative overflow-hidden bg-slate-100">
                        ...
                        {/* Hover overlay */}
                        <div className="absolute inset-0 bg-[#00274D]/50 opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex items-center justify-center">
